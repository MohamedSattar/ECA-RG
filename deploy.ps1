...
# which fails on Windows-generated ZIPs (even with forward-slash fix we bypass it).
...
$pubCreds = az webapp deployment list-publishing-credentials `
    ...
    $(if ($AzureSlot -ne "production") { "--slot $AzureSlot" }) `
...
# Derive the SCM hostname from kuduHostName app property
$scmHost = az webapp show --resource-group $ResourceGroup --name $AppName `
    $(if ($AzureSlot -ne "production") { "--slot $AzureSlot" }) `
    --query "defaultHostName" -o tsv | ForEach-Object {
        $appHost = $_.Trim()
        $appHost -replace "\.azurewebsites\.net$", ".scm.azurewebsites.net"
    }
# Fallback: use the Kudu URL from the webapp resource
if (-not $scmHost) {
    $scmHost = az webapp show --resource-group $ResourceGroup --name $AppName `
        --query "repositorySiteName" -o tsv
}
...
$uploadResp = Invoke-WebRequest -Uri $deployUrl -Method POST `
    -Headers $uploadHeaders -InFile $ZipPath -UseBasicParsing
...
} while (-not $d.complete -and $elapsed -lt $maxWait)

if ($d.status -ne 4) {
    throw "Deployment failed (Kudu status $($d.status)). Check $pollUrl"
}
...
# Derive the real hostname from the webapp (handles regional subdomains like uaenorth-01)
$appUrl = az webapp show --resource-group $ResourceGroup --name $AppName `
    $(if ($AzureSlot -ne "production") { "--slot $AzureSlot" }) `
